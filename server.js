...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    db.run(
      `INSERT INTO users (
        email, password, name, phone, verificationToken, authMethod
      ) VALUES (?, ?, ?, ?, ?, ?)`,
      [email, hashedPassword, name, phone, verificationToken, "email"],
      function (err) {
        if (err) {
          return res
            .status(400)
            .json({ error: "Email или телефон уже зарегистрирован" })
        }
        ...
        if (email) {
          ...
          transporter.sendMail({
            ...
            html: `
              ...
              <p>Пожалуйста, перейдите по ссылке для подтверждения: <a href="${verificationLink}">${verificationLink}</a></p>
            `,
          })
        }
        ...
        res.json({
          message:
            "Регистрация успешна! Проверьте ваш email/телефон для подтверждения",
        })
      }
    )
  } catch (error) {
...
app.post("/api/send-verification-code", async (req, res) => {
  ...
  try {
    ...
    await mts.sendSms({
      phone,
      message: `Ваш код подтверждения Dominik: ${code}`,
    })

    db.run(
      "UPDATE users SET verificationCode = ?, verificationCodeExpires = ? WHERE phone = ?",
      [code, Date.now() + 3600000, phone]
    )
...
app.post("/api/verify-phone", (req, res) => {
  ...
  db.get(
    "SELECT * FROM users WHERE phone = ? AND verificationCode = ? AND verificationCodeExpires > ?",
    [phone, code, Date.now()],
    async (err, user) => {
      if (err || !user) {
        return res
          .status(400)
          .json({ error: "Неверный код или срок действия истек" })
      }
